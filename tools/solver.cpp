...
const char* argStr(int argc, char** argv, const char* flag, const char* def) {
    for (int i = 1; i + 1 < argc; ++i)
...
// Emit the winnable-pool bitmap as a C header consumed by src/game.cpp.
void writePoolHeader(const char* path, uint32_t bits, uint32_t count, size_t budget,
                     const std::vector<uint8_t>& bm) {
    std::FILE* f = std::fopen(path, "w");
    if (!f) {
        std::fprintf(stderr, "genpool: cannot open %s for writing\n", path);
        std::abort();
    }
    std::fprintf(f,
        ...
        "// do not edit by hand.\n"
        "//\n"
        "// Scanned seeds [0, %u) at a %zu-node solver budget: %u winnable (%.1f%%).\n"
        "#pragma once\n"
        "#include <cstdint>\n",
        bits, budget, count, bits ? 100.0 * count / bits : 0.0);
    std::fprintf(f, "static const uint32_t kWinnablePoolBits = %u;\n", bits);
    std::fprintf(f, "static const uint32_t kWinnablePoolCount = %u;\n", count);
    std::fprintf(f, "static const uint32_t kWinnablePoolBudget = %u;\n", (uint32_t)budget);
    std::fprintf(f, "static const unsigned char kWinnablePool[] = {");
    for (size_t i = 0; i < bm.size(); ++i) {
        if (i % 16 == 0) std::fprintf(f, "\n    ");
        std::fprintf(f, "0x%02x,", bm[i]);
    }
    std::fprintf(f, "\n};\n");
    std::fclose(f);
}

// Scan seeds [0, bits), prove winnability at `budget`, and write the bitmap header.
// Work is claimed in small blocks via an atomic cursor so the huge per-deal time
// variance (easy wins vs. budget-capped deals) load-balances across threads.
int genPool(uint32_t bits, size_t budget, int threads, const char* out) {
    std::printf("Sawayama Solitaire — winnable-pool generation\n");
    std::printf("scanning seeds [0, %u) at %zu states/deal on %d threads -> %s\n\n",
                bits, budget, threads, out);

    std::vector<uint8_t> win(bits, 0);  // 1 byte/seed during the scan (no bit races)
    std::atomic<uint32_t> nextBlock{0};
    std::atomic<uint32_t> done{0};
    const uint32_t BLOCK = 32;
    const uint32_t numBlocks = (bits + BLOCK - 1) / BLOCK;
    auto t0 = std::chrono::steady_clock::now();

    auto worker = [&] {
        for (;;) {
            uint32_t b = nextBlock.fetch_add(1, std::memory_order_relaxed);
            if (b >= numBlocks) break;
            uint32_t lo = b * BLOCK, hi = std::min(bits, lo + BLOCK);
            for (uint32_t seed = lo; seed < hi; ++seed)
                if (solve(dealSeed(seed), budget) == SOLVED) win[seed] = 1;
    ...
    uint32_t d;
    while ((d = done.load(std::memory_order_relaxed)) < bits) {
        std::this_thread::sleep_for(std::chrono::milliseconds(1000));
        d = done.load(std::memory_order_relaxed);
        double el = std::chrono::duration<double>(std::chrono::steady_clock::now() - t0).count();
        double rate = el > 0 ? d / el : 0;
        double eta = rate > 0 ? (bits - d) / rate : 0;
        std::fprintf(stderr, "\r  %u/%u (%.1f%%)  %.0f seeds/s  ETA %.0fs        ",
                     d, bits, 100.0 * d / bits, rate, eta);
    }
    for (auto& th : pool) th.join();
    std::fprintf(stderr, "\r%-72s\r", "");

    size_t nbytes = (bits + 7) / 8;
    std::vector<uint8_t> bm(nbytes, 0);
    uint32_t count = 0;
    ...
    writePoolHeader(out, bits, count, budget, bm);

    double secs = std::chrono::duration<double>(std::chrono::steady_clock::now() - t0).count();
    std::printf("done: %u/%u winnable (%.1f%%), %zu-byte bitmap -> %s  [%.0fs, %.0f seeds/s]\n",
                count, bits, 100.0 * count / bits, nbytes, out, secs, secs > 0 ? bits / secs : 0);
    return 0;
}
...
int main(int argc, char** argv) {
    ...
    // Pool generation: scan seeds [0, --bits) and write a winnable-deal bitmap.
    if (hasFlag(argc, argv, "--genpool")) {
        uint32_t bits = (uint32_t)argLong(argc, argv, "--bits", 131072);
        const char* out = argStr(argc, argv, "--out", "src/pool_data.h");
        int gthreads = (int)argLong(argc, argv, "--threads", 0);
        if (gthreads <= 0) gthreads = (int)std::max(1u, std::thread::hardware_concurrency());
        return genPool(bits, budget, gthreads, out);
    }
